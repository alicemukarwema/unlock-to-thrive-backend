import Career from "../models/career.model.js";
import cloudinary from "../config/cloudinary.js";
import fs from "fs";

export const create = async (req, res) => {
  try {
    const { 
      title,
      category, 
      level, 
      description, 
      instructor, 
      rating,
      mentorshipDuration,
      weeklyCommitment,
      maxMentees,
      requiredSkills,
      recommendedResources,
      careerPathways,
      industryTrends,
      professionalNetworks
    } = req.body;

    // Default image path if no file uploaded
    let imagePath = "/default-career.jpg";

    // If file was uploaded, multer-storage-cloudinary has already uploaded it
    if (req.file) {
      imagePath = req.file.path; 
    }

    const safeParseJSON = (data, defaultValue = []) => {
      try {
        return data ? JSON.parse(data) : defaultValue;
      } catch (error) {
        console.error("JSON parsing error:", error);
        return defaultValue;
      }
    };
    
    const newCareer = new Career({
      title,
      category,
      level,
      description,
      instructor,
      rating: parseFloat(rating) || 0,
      imagePath,
      mentorshipDuration: mentorshipDuration || "3 months",
      weeklyCommitment: weeklyCommitment || "2 hours",
      maxMentees: parseInt(maxMentees) || 5,
      requiredSkills: safeParseJSON(requiredSkills),
      recommendedResources: safeParseJSON(recommendedResources),
      careerPathways: safeParseJSON(careerPathways),
      industryTrends: safeParseJSON(industryTrends),
      professionalNetworks: safeParseJSON(professionalNetworks)
    });

    const savedCareer = await newCareer.save();
    res.status(201).json(savedCareer);
  } catch (error) {
    console.log("hdxdjdxk",error)
    res.status(500).json({ error: error.message });
  }
};

export const update = async (req, res) => {
  try {
    const careerId = req.params.id;
    const { 
      title, 
      category, 
      level, 
      description, 
      instructor, 
      rating,
      mentorshipDuration,
      weeklyCommitment,
      maxMentees,
      requiredSkills,
      recommendedResources,
      careerPathways,
      industryTrends,
      professionalNetworks
    } = req.body;

    const career = await Career.findById(careerId);
    if (!career) {
      return res.status(404).json({ message: "Career not found" });
    }

    let imagePath = career.imagePath;

    // If new file was uploaded, multer-storage-cloudinary has already uploaded it
    if (req.file) {
      // Get old image public ID to delete if it's not the default
      if (career.imagePath && career.imagePath !== "data:image/jpeg;base64," && career.imagePath.includes("cloudinary")) {
        try {
          const publicId = career.imagePath.split("/").pop().split(".")[0];
          await cloudinary.uploader.destroy(`careers/${publicId}`);
        } catch (deleteError) {
          console.error("Error deleting old image:", deleteError);
        }
      }
      
      // Update with new image path
      imagePath = req.file.path; // This will be the Cloudinary URL
    }

    const updatedCareer = await Career.findByIdAndUpdate(
      careerId,
      { 
        title, 
        category, 
        level, 
        description, 
        instructor, 
        rating: parseFloat(rating) || 0, 
        imagePath,
        mentorshipDuration: mentorshipDuration || career.mentorshipDuration,
        weeklyCommitment: weeklyCommitment || career.weeklyCommitment,
        maxMentees: parseInt(maxMentees) || career.maxMentees,
        requiredSkills: requiredSkills ? JSON.parse(requiredSkills) : career.requiredSkills,
        recommendedResources: recommendedResources ? JSON.parse(recommendedResources) : career.recommendedResources,
        careerPathways: careerPathways ? JSON.parse(careerPathways) : career.careerPathways,
        industryTrends: industryTrends ? JSON.parse(industryTrends) : career.industryTrends,
        professionalNetworks: professionalNetworks ? JSON.parse(professionalNetworks) : career.professionalNetworks
      },
      { new: true }
    );

    res.status(200).json(updatedCareer);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};

// Additional methods remain the same as in the previous implementation
export const findAll = async (req, res) => {
  try {
    const { search, category, level, page = 1, limit = 10 } = req.query;

    const filter = {};

    if (search) {
      filter.$text = { $search: search };
    }

    if (category && category !== "all") {
      filter.category = category;
    }

    if (level && level !== "all") {
      filter.level = level;
    }

    const skip = (parseInt(page) - 1) * parseInt(limit);

    const careers = await Career.find(filter)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(parseInt(limit));

    const total = await Career.countDocuments(filter);

    res.status(200).json({
      careers,
      totalPages: Math.ceil(total / parseInt(limit)),
      currentPage: parseInt(page),
      total,
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};

export const findOne = async (req, res) => {
  try {
    const career = await Career.findById(req.params.id);

    if (!career) {
      return res.status(404).json({ message: "Career not found" });
    }

    res.status(200).json(career);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};

export const deleteCareer = async (req, res) => {
  try {
    const careerId = req.params.id;

    const career = await Career.findById(careerId);
    if (!career) {
      return res.status(404).json({ message: "Career not found" });
    }

    if (career.imagePath && career.imagePath !== "/default-career.jpg" && career.imagePath.includes("cloudinary")) {
      const publicId = career.imagePath.split("/").pop().split(".")[0];
      await cloudinary.uploader.destroy(`careers/${publicId}`);
    }

    await Career.findByIdAndDelete(careerId);

    res.status(200).json({ message: "Career deleted successfully" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};

// New method to get detailed resources for a career
// export const getCareerResources = async (req, res) => {
//   try {
//     const careerId = req.params.id;
//     const career = await Career.findById(careerId);

//     if (!career) {
//       return res.status(404).json({ message: "Career not found" });
//     }

//     res.status(200).json({
//       requiredSkills: career.requiredSkills,
//       recommendedResources: career.recommendedResources,
//       careerPathways: career.careerPathways,
//       industryTrends: career.industryTrends,
//       professionalNetworks: career.professionalNetworks
//     });
//   } catch (error) {
//     res.status(500).json({ error: error.message });
//   }
// };
export const addResourceToCareers = async (req, res) => {
  try {   
    const { careerId } = req.params;
    const resourceData = req.body;

    // Validate input
    if (!careerId) {
      return res.status(400).json({ error: "Career ID is required" });
    }

    // Find the career
    const career = await Career.findById(careerId);
    
    if (!career) {
      return res.status(404).json({ error: "Career not found" });
    }

    // Determine which array to push to based on resource category
    switch(resourceData.category) {
      case 'Required Skills':
        if (!career.requiredSkills) career.requiredSkills = [];
        career.requiredSkills.push({
          name: resourceData.title,
          description: resourceData.description,
          learningResources: [resourceData]
        });
        break;
      case 'Recommended Resources':
        if (!career.recommendedResources) career.recommendedResources = [];
        career.recommendedResources.push(resourceData);
        break;
      case 'Career Pathways':
        if (!career.careerPathways) career.careerPathways = [];
        career.careerPathways.push({
          title: resourceData.title,
          description: resourceData.description
        });
        break;
      case 'Industry Trends':
        if (!career.industryTrends) career.industryTrends = [];
        career.industryTrends.push({
          trend: resourceData.title,
          description: resourceData.description
        });
        break;
      case 'Professional Networks':
        if (!career.professionalNetworks) career.professionalNetworks = [];
        career.professionalNetworks.push({
          name: resourceData.title,
          url: resourceData.url,
          description: resourceData.description
        });
        break;
      default:
        return res.status(400).json({ error: "Invalid resource category" });
    }

    // Save the updated career
    const updatedCareer = await career.save();

    res.status(201).json({
      message: "Resource added successfully",
      career: updatedCareer
    });

  } catch (error) {
    console.error("Error adding resource:", error);
    res.status(500).json({ 
      error: "Internal server error", 
      details: error.message 
    });
  }
};

// Update an existing resource
export const updateCareerResource = async (req, res) => {
  try {
    const { careerId, resourceId } = req.params;
    const { 
      title, 
      type, 
      url, 
      description, 
      category 
    } = req.body;

    const career = await Career.findById(careerId);
    if (!career) {
      return res.status(404).json({ message: "Career not found" });
    }

    const resourceIndex = career.resources.findIndex(
      resource => resource._id.toString() === resourceId
    );

    if (resourceIndex === -1) {
      return res.status(404).json({ message: "Resource not found" });
    }

    // Update resource
    career.resources[resourceIndex] = {
      ...career.resources[resourceIndex],
      title,
      type,
      url,
      description,
      category
    };

    await career.save();

    res.status(200).json({
      message: "Resource updated successfully",
      resource: career.resources[resourceIndex]
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};

// Delete a resource from a career
export const deleteCareerResource = async (req, res) => {
  try {
    const { careerId, resourceId } = req.params;

    const career = await Career.findById(careerId);
    if (!career) {
      return res.status(404).json({ message: "Career not found" });
    }

    career.resources = career.resources.filter(
      resource => resource._id.toString() !== resourceId
    );

    await career.save();

    res.status(200).json({ 
      message: "Resource deleted successfully" 
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};

// Modify existing getCareerResources to return all resources
export const getCareerResources = async (req, res) => {
  try {
    const career = await Career.findById(req.params.id);

    if (!career) {
      return res.status(404).json({ message: "Career not found" });
    }

    // Directly return the existing resource fields
    const groupedResources = {
      requiredSkills: career.requiredSkills || [],
      recommendedResources: career.recommendedResources || [],
      careerPathways: career.careerPathways || [],
      industryTrends: career.industryTrends || [],
      professionalNetworks: career.professionalNetworks || []
    };

    res.status(200).json(groupedResources);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};